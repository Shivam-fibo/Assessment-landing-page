    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-2xl bg-[#2B1330]/95 backdrop-blur-xl px-6 py-6 border border-white/15 shadow-2xl text-white",
            className,
          )}
        >
          {children}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX className="text-white cursor-pointer hover:opacity-80 transition-opacity" onClick={onClick} />
  ) : (
    <IconMenu2 className="text-white cursor-pointer hover:opacity-80 transition-opacity" onClick={onClick} />
  );
};
